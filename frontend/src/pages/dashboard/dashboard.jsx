...
`;
    ...
    try {
        const response = await onCreateNote(defaultNoteTitle, null);
        if (response.status === 201) {
            const note_id = response.data.noteCreated.note_id;
            await updateNoteContent(note_id, defaultNoteContent);
            setListOfNotes([...listOfNotes, response.data.noteCreated]);
        }
    } catch (error) {
        console.log('Error creating default note:', error);
    }
    };
